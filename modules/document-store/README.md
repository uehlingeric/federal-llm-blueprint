...
| force\_destroy | Allow terraform destroy to empty the buckets (all object versions) first. Sandbox teardown aid — log-delivery buckets receive objects continuously, so destroy reliably fails without it. Production keeps false; incompatible with the intent of object lock. | `bool` | `false` | no |
